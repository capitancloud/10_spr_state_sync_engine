...
const getLogStyles = (type: OperationLog['type']) => {
  const styles = {
    optimistic: {
      ...
      icon: 'text-primary',
      label: '⚡ STEP 1: Aggiornamento Immediato',
      labelBg: 'bg-primary/20',
      explanation: 'L\'interfaccia si aggiorna SUBITO, senza aspettare il server!'
    },
    backend: {
      bg: 'bg-muted/50',
      border: 'border-border',
      icon: 'text-muted-foreground',
      label: '🔄 STEP 2: Sincronizzazione',
      labelBg: 'bg-muted',
      explanation: 'Stiamo inviando i dati al server in background...'
    },
    rollback: {
      bg: 'bg-gradient-to-r from-warning/10 to-orange-500/10',
      border: 'border-warning/30',
      icon: 'text-warning',
      label: '↩️ Rollback Automatico',
      labelBg: 'bg-warning/20',
      explanation: 'Ops! Qualcosa è andato storto, torniamo allo stato precedente.'
    },
    success: {
      bg: 'bg-gradient-to-r from-success/10 to-emerald-500/10',
      border: 'border-success/30',
      icon: 'text-success',
      label: '✅ STEP 3: Confermato!',
      labelBg: 'bg-success/20',
      explanation: 'Il server ha confermato: i dati sono salvati in modo permanente.'
    },
    error: {
      ...
      explanation: 'Il server ha rifiutato l\'operazione. Tra poco vedremo il rollback...'
    },
  };
  return styles[type];
};
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-3 rounded-lg bg-primary/5 border border-primary/20 mb-4">
        <p className="text-sm text-muted-foreground leading-relaxed">
          <span className="font-semibold text-primary">👀 Cosa vedrai qui:</span> Ogni volta che fai un'azione (aggiungi, completa, elimina una task), 
          qui apparirà ESATTAMENTE cosa succede dietro le quinte, passo dopo passo. 
          È come guardare "dentro" l'applicazione!
        </p>
      </div>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Spiegazione automatica del tipo di log */}
                      <motion.p 
                        initial={{ opacity: 0, height: 0 }}
                        animate={{ opacity: 1, height: 'auto' }}
                        className="text-xs text-muted-foreground mt-2 p-2 rounded-lg bg-muted/30 italic"
                      >
                        {styles.explanation}
                      </motion.p>
                      ...
                      {log.details && (
                        ...
                        >
                          💡 <strong>Dettaglio:</strong> {log.details}
                        </motion.p>
                      )}
                    </div>
                  </div>
